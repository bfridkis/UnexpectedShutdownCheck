...
if($results.count -gt 0) { $results | Sort-Object 'Unexpected Reboot Count (Event ID 6008)', Hostname | ConvertTo-CSV -NoTypeInformation | Add-Content -Path $outputFile }
else { Add-Content -Path $outputFile -Value "No nodes found with unexpected shutdown (Event ID 6008) within the last $daysToCheck days." }

Write-Host "Servers With Unexpected Reboot (Event ID 6008) Within The Last $daysToCheck Days:"
